...
export class KnifeTool {
  ...
  computeNewVertices(a, b, meshData) {
    ...
    for (let edge of meshData.edges.values()) {
      ...
      const p1 = new THREE.Vector3(v1.position.x, v1.position.y, v1.position.z);
      const p2 = new THREE.Vector3(v2.position.x, v2.position.y, v2.position.z);
      ...
      if (withinSegment) {
        this.newVertices.push(intersection);
      }
    }
  }

  applyCut() {
    if (this.newVertices.length < 1) return;
    ...
    for (const v of this.newVertices) {
      addedVertices.push(meshData.addVertex({ x: v.x, y: v.y, z: v.z }));
    }
    ...
    this.editSelection.selectVertices(addedVertices.map(v => v.id));
    this.cancelCut();
  }
